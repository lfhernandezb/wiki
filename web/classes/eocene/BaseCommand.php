...
class BaseCommand {
	...
	function initBase(){
		...
		switch($engineType){
			...
			case "EOCENETEMPLATEENGINE":
				...
				break;
			default:
				$fc->writeError("Error: No template engine found");
		}
	}
	...
	function addVarC($tokenName,$theValue){
		$this->templateEngine->setVar($tokenName,$theValue);
	}
	...
	function processTemplate($templateAlias){
		...
		if(!isset($fc->templates[$templateAlias])) $fc->writeError("Fatal: No template found for template alias $templateAlias");
		$this->_processTemplate($fc->templates[$templateAlias]);
	}
	
	function processDB($id) {
		global $fc;
		$this->templateEngine->processDB($id);
		$fc->response->write($this->templateEngine->fileContents);
	}
	
	function processVar($var) {
		$this->templateEngine->processVar($var);
		$fc->response->write($this->templateEngine->fileContents);
	}
	...
	function &getContents(){
		global $fc;
		$fc->writeError("&getContents() method should be overwritten in subclass");
	}
	...
	*******************************************************************************/
	function execute(){
		global $fc;
		$c=&$fc->commandClass;
		$fc->writeError("Must overwrite execute method in command $c or one of the classes used as plugs in templates of $c");
	}
