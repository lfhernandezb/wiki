...
class MySQL
{
	...
	*/
	...
	public function Open($database = null, $server = null, $username = null,
		...
		// flag 128 to let client do 'LOAD DATA LOCAL'
		if ($pcon) {
			$this->mysql_link = @mysql_pconnect(
				$this->db_host, $this->db_user, $this->db_pass, 128);
		} else {
			$this->mysql_link = @mysql_connect (
				$this->db_host, $this->db_user, $this->db_pass, false, 128);
		}
	...
	private function SetError($errorMessage = "", $errorNumber = 0) {
		...
		if (isset($this->error_desc) && $this->error_desc != NULL) {
	        $trace = debug_backtrace();
	        trigger_error(
	            "MySQL Error Number: {$this->error_number}; Description: {$this->error_desc}",
	            E_USER_NOTICE);
		}
